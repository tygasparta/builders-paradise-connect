  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Point of Sale — Builders Paradise ERP" },
      {
        ...
        content: "Fast counter checkout with barcode entry, multi-tender payment and live stock deduction.",
      },
    ],
  }),
  component: POS,
            ...
            {catalogue.map((p) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <Badge variant="outline" className="num border-border text-[10px] text-muted-foreground">
                    {p.sku}
                  </Badge>
      ...
      <aside className="card-surface flex min-h-[640px] flex-col overflow-hidden">
          ...
          <div className="space-y-2 p-4">
            {!lines.length && (
              <div className="py-20 text-center">
                ...
                <p className="mt-3 text-sm text-muted-foreground">Scan or tap products to begin</p>
              </div>
            )}
            {lines.map((l) => (
        ...
        <div className="space-y-3 border-t border-border p-4">
          <div className="flex items-center gap-2">
            ...
            <span className="flex-1 text-xs text-muted-foreground">Discount % (needs approval)</span>
            <Input
              type="number"
              min={0}
              max={20}
              value={discount}
              onChange={(e) => setDiscount(Math.min(20, Math.max(0, Number(e.target.value) || 0)))}
          ...
          <div className="num space-y-1.5 text-sm">
            ...
            {discount > 0 && <Row label={`Discount (${discount}%)`} value={`-${money(discountValue)}`} />}
